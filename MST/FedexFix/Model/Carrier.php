...
class Carrier extends \Magento\Fedex\Model\Carrier 
{
    ...
    public function __construct(
        ...
        $wsdlRateBasePath = $configReader->getModuleDir(Dir::MODULE_ETC_DIR, 'MST_FedexFix') . '/wsdl/';
        $this->_shipServiceWsdl = $wsdlBasePath . 'ShipService_v10.wsdl';
        $this->_rateServiceWsdl = $wsdlRateBasePath . 'RateService_v31.wsdl';
    ...
    {
        return ['ServiceId' => 'crs', 'Major' => '31', 'Intermediate' => '0', 'Minor' => '0'];
    }
    ...
    protected function _getRateAmountOriginBased($rate)
    {
        ...
        if (is_object($rate)) {
          ...
          if( is_array($rate->RatedShipmentDetails) && count($rate->RatedShipmentDetails) > 0) {
            foreach ($rate->RatedShipmentDetails as $ratedShipmentDetail) {
                ...
                $netAmount = (string)$rate->RatedShipmentDetails->ShipmentRateDetail->TotalNetCharge->Amount;
                $currencyCode = (string)$rate->RatedShipmentDetails->ShipmentRateDetail->TotalNetCharge->Currency;
                $rateType = (string)$rate->RatedShipmentDetails->ShipmentRateDetail->RateType;
                $rateTypeAmounts[$rateType] = $netAmount;
          }
    ...
    private function getBaseCurrencyRate(string $currencyCode): float
    {
        if (!isset($this->baseCurrencyRate[$currencyCode])) {
            $currencyCode = $this->convertFedexCurrency($currencyCode);
    ...
    public function getCode($type, $code = '')
    {
        $codes = [
            'method' => [
                ...
                'FEDEX_NEXT_DAY_END_OF_DAY' => __('Next Day - End of Day'),
                'FEDEX_NEXT_DAY_MID_MORNING' => __('Next Day - Mid Morning'),
                'FEDEX_DISTANCE_DEFERRED' => __('Economy'),
                'FEDEX_NEXT_DAY_AFTERNOON' => __('Next Day - Afternoon'),
                'FEDEX_INTERNATIONAL_PRIORITY' => __('International Priority'),
                'FEDEX_REGIONAL_ECONOMY' => __('Regional Economy'),
                'FEDEX_INTERNATIONAL_CONNECT_PLUS' => __('International Connect Plus'),
            ],
            'dropoff' => [
   ...
   /**
    * Array of fedex Codes -> normal people codes
    */
    public function getFedexCurrency() {
      $codes = [
        'UKL' => 'GBP',
        'RDD' => 'DOP',
        'ECD' => 'XCD',
   ...
   /**
    * Convert Fedex Currency Codes to ISO codes
    * Because America?
    */
   public function convertFedexCurrency($currencyCode) {
        $fedexCodes = $this->getFedexCurrency();

        if( array_key_exists($currencyCode, $fedexCodes) ) {
            $currencyCode = $fedexCodes[$currencyCode];
        }
        return $currencyCode;
   }
